...
int klimtlte_boot(struct ipc_client *client)
{
    ...
    modem_image_data = mmap(0, KLIMTLTE_MODEM_IMAGE_SIZE, PROT_READ, MAP_SHARED, modem_image_fd, 0);
    ...
    p = (unsigned char *) modem_image_data + KLIMTLTE_PSI_OFFSET;

    rc = xmm626_hsic_psi_send(client, modem_boot_fd, (void *) p, KLIMTLTE_PSI_SIZE);
    if (rc < 0) {
    ...
    p = (unsigned char *) modem_image_data + KLIMTLTE_EBL_OFFSET;

    rc = xmm626_hsic_ebl_send(client, modem_boot_fd, (void *) p, KLIMTLTE_EBL_SIZE);
    if (rc < 0) {
    ...
    p = (unsigned char *) modem_image_data + KLIMTLTE_SEC_START_OFFSET;

    rc = xmm626_hsic_sec_start_send(client, modem_boot_fd, (void *) p, KLIMTLTE_SEC_START_SIZE);
    if (rc < 0) {
    ...
    p = (unsigned char *) modem_image_data + KLIMTLTE_FIRMWARE_OFFSET;

    rc = xmm626_hsic_firmware_send(client, modem_boot_fd, (void *) p, KLIMTLTE_FIRMWARE_SIZE);
    if (rc < 0) {
...
complete:
    if (modem_image_data != NULL)
        munmap(modem_image_data, KLIMTLTE_MODEM_IMAGE_SIZE);
...
int klimtlte_open(void *data, int type)
{
    struct klimtlte_transport_data *transport_data;

    if (data == NULL)
        return -1;

    transport_data = (struct klimtlte_transport_data *) data;
...
int klimtlte_read(void *data, void *buffer, size_t length)
{
    struct klimtlte_transport_data *transport_data;
    int rc;
...
int klimtlte_write(void *data, const void *buffer, size_t length)
{
    struct klimtlte_transport_data *transport_data;
    int rc;
...
int klimtlte_poll(void *data, struct ipc_poll_fds *fds, struct timeval *timeout)
{
    struct klimtlte_transport_data *transport_data;
    int rc;
...
int klimtlte_power_on(void *data)
{
    return 0;
}

int klimtlte_power_off(void *data)
{
    int fd;
    int rc;
...
int klimtlte_gprs_activate(void *data, unsigned int cid)
{
    return 0;
}

int klimtlte_gprs_deactivate(void *data, unsigned int cid)
{
    return 0;
}

int klimtlte_data_create(void **transport_data, void **power_data,
    void **gprs_data)
{
    if (transport_data == NULL)
        return -1;

    *transport_data = calloc(1, sizeof(struct klimtlte_transport_data));

    return 0;
}

int klimtlte_data_destroy(void *transport_data, void *power_data,
    void *gprs_data)
{
    if (transport_data == NULL)
        return -1;
...
struct ipc_client_ops klimtlte_fmt_ops = {
    .boot = klimtlte_boot,
    .send = xmm626_sec_modem_fmt_send,
    .recv = xmm626_sec_modem_fmt_recv,
};

struct ipc_client_ops klimtlte_rfs_ops = {
    .boot = NULL,
    .send = xmm626_sec_modem_rfs_send,
    .recv = xmm626_sec_modem_rfs_recv,
};

struct ipc_client_handlers klimtlte_handlers = {
    .read = klimtlte_read,
    .write = klimtlte_write,
    .open = klimtlte_open,
    .close = klimtlte_close,
    .poll = klimtlte_poll,
    .transport_data = NULL,
    .power_on = klimtlte_power_on,
    .power_off = klimtlte_power_off,
    .power_data = NULL,
    .gprs_activate = klimtlte_gprs_activate,
    .gprs_deactivate = klimtlte_gprs_deactivate,
    .gprs_data = NULL,
    .data_create = klimtlte_data_create,
    .data_destroy = klimtlte_data_destroy,
};

struct ipc_client_gprs_specs klimtlte_gprs_specs = {
    .gprs_get_iface = xmm626_sec_modem_gprs_get_iface,
    .gprs_get_capabilities = xmm626_sec_modem_gprs_get_capabilities,
};

struct ipc_client_nv_data_specs klimtlte_nv_data_specs = {
